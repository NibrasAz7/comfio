...
class TestAugmentTSVCDF:
    def test_returns_correct_length(self, mock_tsv_votes, mock_tsv_timestamps, mock_target_timestamps):
        result = augment_tsv_cdf(
            mock_tsv_votes, mock_tsv_timestamps, mock_target_timestamps,
        )
        assert len(result) == len(mock_target_timestamps)

    def test_preserves_value_range(self, mock_tsv_votes, mock_tsv_timestamps, mock_target_timestamps):
        result = augment_tsv_cdf(
            mock_tsv_votes, mock_tsv_timestamps, mock_target_timestamps,
        )
    ...
    def test_mismatched_lengths_raises(self, mock_target_timestamps):
        with pytest.raises(ValueError, match="same length"):
            augment_tsv_cdf(
                np.array([1, 2, 3]), np.array([0, 1]), mock_target_timestamps,
            )

    def test_single_target(self, mock_tsv_votes, mock_tsv_timestamps):
        result = augment_tsv_cdf(
            mock_tsv_votes, mock_tsv_timestamps, np.array([5.0]),
        )
        assert len(result) == 1
...
class TestEvaluateTSV:
    ...
    def test_compliance(self):
        ...
        # |TSV| <= 1.5 → compliant
        assert result.compliant[0] == True   # 0
        assert result.compliant[1] == True   # 1
        assert result.compliant[3] == False  # 2
        assert result.compliant[5] == False  # 3
    ...
    def test_custom_threshold(self):
        ...
        assert result.compliant[0] == True   # |1| <= 2
        assert result.compliant[1] == True   # |2| <= 2
        assert result.compliant[2] == False  # |3| > 2
